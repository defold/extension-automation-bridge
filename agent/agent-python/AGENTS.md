...
- Prefer `AgentClient.from_project(".", build=True)` for normal scripts; it closes known candidate engine ports, builds, waits for `Defold Agent endpoint registered`, discovers the engine service port, and waits for `/agent/v1/health`.
...
- Use `agent.click(...)`, `agent.drag(..., duration=...)`, `agent.type_text(...)`, and `agent.key("KEY_ENTER")`; drag blocks until the queued drag should be finished by default.
