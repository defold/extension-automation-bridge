# Native API Agent Notes

- Use this raw HTTP API when you need curl-level access or are implementing a wrapper; otherwise prefer `agent/agent-python`.
- Discover the engine service port from the editor console line `Engine service started on port <port>`.
- Base URL: `http://127.0.0.1:<engine_service_port>/agent/v1`.
